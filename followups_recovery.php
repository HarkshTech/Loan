...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id']) && isset($_POST['remarks']) && isset($_POST['visitNeeded']) && isset($_POST['visitSchedule'])) {
    ...
    // Get the existing remarks
    $sqlGetRemarks = "SELECT Remarks FROM recovery_data WHERE ID = ? AND CaseStatus='Recovery Done'";
    ...
    $stmt->bind_param("ssssi", $updatedRemarks, $visitNeeded, $visitSchedule, $visitStatus, $id);
...
<head>
    ...
    <title>Follow-ups Management</title>
...
<body>
    <div class="container">
        ...
        <h1 class="mb-4" style="font-size: 20px;line-height: 27px;">Follow-ups Management</h1>
                    ...
                    if ($loggedInUser) {
                        ...
                        if ($_SESSION['role'] === 'recovery') {
                            $sql = "SELECT rd.*,pi.ID leadid, pi.FullName, pi.PhoneNumber, pi.Address, es.EMIAmount
                                    ...
                                    WHERE rd.AssignedTo = ? AND rd.Remarks IS NOT NULL AND rd.CaseStatus<>'Recovery Done'";
                        } else {
                            $sql = "SELECT rd.*,pi.ID leadid, pi.FullName, pi.PhoneNumber, pi.Address, es.EMIAmount
                                    FROM recovery_data rd
                                    JOIN personalinformation pi ON rd.LeadID = pi.ID
                                    LEFT JOIN emi_schedule es ON rd.LeadID = es.LeadID
                                    WHERE rd.Remarks IS NOT NULL AND rd.CaseStatus<>'Recovery Done'";
                        }
