        ...
        switch ($paymentType) {
            case 'collect_payment':
                if ($paymentType === 'collect_payment') {
                    ...
                    if ($conn->query($updateSql) === TRUE) {
                        $fetchSql = "SELECT EMIAmount, overdue_days, NextPaymentDate, PaidEMIs, TotalEMIs FROM emi_schedule WHERE LeadID = $leadId";
                        ...
                        $insertSql = "INSERT INTO emi_payments (LeadID, PaymentDate, EMIAmount, PaymentType, OverdueDays, Status, PaymentReceiver, ReceiverDetails, bmapproval, superapproval, collector) 
                                      VALUES ('$leadId', NOW(), '$emiAmount', '$paymentType', '$overdueDays', '$status', '$paymentReceiver', '$receiverDetails', '0', '0', '$user')";
                        if ($conn->query($insertSql) === TRUE) {
                            $updateDatesSql = "";
            
                            if ($paidEMIs == 1) {
                                // Only for the first payment, check the NextPaymentDate
                                $nextPaymentDay = date('j', strtotime($nextPaymentDate));
                                if ($nextPaymentDay > 15) {
                                    // Set LastPaymentDate to NextPaymentDate and NextPaymentDate to the 5th of the month after next
                                    $nextMonth = date('Y-m-05', strtotime('+2 months', strtotime($nextPaymentDate)));
                                    $updateDatesSql = "UPDATE emi_schedule 
                                                      SET LastPaymentDate = '$nextPaymentDate', 
                                                          NextPaymentDate = '$nextMonth' 
                                                      WHERE LeadID = $leadId";
                                } else {
                                    // Set NextPaymentDate to the 5th of the next month from old NextPaymentDate
                                    $nextMonth = date('Y-m-05', strtotime('+1 month', strtotime($nextPaymentDate)));
                                    $updateDatesSql = "UPDATE emi_schedule 
                                ...
                                // For subsequent payments, update NextPaymentDate to the 5th of the next month from old NextPaymentDate
                                $nextMonth = date('Y-m-05', strtotime('+1 month', strtotime($nextPaymentDate)));
                                $updateDatesSql = "UPDATE emi_schedule 
                                                  SET LastPaymentDate = NOW(), 
                                                      NextPaymentDate = '$nextMonth' 
            ...
            case 'partial_payment':
                ...
                if ($paymentType === 'partial_payment') {
                    ...
                    $fetchSql = "SELECT EMIAmount, PartialPayment, NextPaymentDate, PaidEMIs, TotalEMIs FROM emi_schedule WHERE LeadID = '$leadId'";
                    $fetchResult = mysqli_query($conn, $fetchSql);
                    $row = mysqli_fetch_assoc($fetchResult);
                    $emiAmount = $row['EMIAmount'];
                    ...
                    if ($newPartialPayment >= $emiAmount) {
                        ...
                        if ($conn->query($updateSql) === TRUE) {
                            $status = 'Paid';
                            $insertSql = "INSERT INTO emi_payments (LeadID, PaymentDate, EMIAmount, PaymentType, OverdueDays, Status, PaymentReceiver, ReceiverDetails, bmapproval, superapproval, collector) 
                                          VALUES ('$leadId', NOW(), '$emiAmount', 'Partial Payment Adjusted in EMI', 0, '$status', '$paymentReceiver', '$receiverDetails', '0', '0', '$user')";
                            if ($conn->query($insertSql) === TRUE) {
                                // Update NextPaymentDate based on the old NextPaymentDate + next month's 5th, only if it's the first payment
                                if ($paidEMIs == 0) {
                                    $nextPaymentDay = date('j', strtotime($nextPaymentDate));
                                    if ($nextPaymentDay > 15) {
                                        // Set LastPaymentDate to NextPaymentDate and NextPaymentDate to the 5th of the month after next
            ...
            case 'penalty':
                ...
                $penaltyAmount = intval($_POST['penaltyAmount']);
                $status='Paid';
                ...
                $updatepayments="INSERT INTO emi_payments (LeadID,PaymentDate,PaymentType,Status,PaymentReceiver,ReceiverDetails,bmapproval,superapproval,remarks) VALUES('$leadId',NOW(),'$paymentType','$status','$paymentReceiver','$receiverDetails','0','0','$penaltyRemarks')";
    ...
    <div class="container" style="margin-top:8%;">
        ...
        <div class="table-responsive">
            <table class="table table-striped" id="tableContainer">
                ...
                <tbody>
                    <?php
                    $sql = "SELECT e.ID, e.LeadID, e.LoanAmount, e.EMIAmount,e.PartialPayment,e.PenaltyAmount, p.FullName, e.NextPaymentDate
                            FROM emi_schedule e
