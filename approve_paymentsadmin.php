...
if (isset($_POST['approve']) && isset($_POST['LeadID'])) {
    ...
    $sql = "UPDATE emi_payments SET superapproval = '1', remarks ='' WHERE PaymentID = ?";
...
if (isset($_POST['reject']) && isset($_POST['remarks']) && isset($_POST['LeadID'])) {
    ...
    $sql33="SELECT PaymentType FROM emi_payments WHERE PaymentID=$paymentID";
    ...
    try {
        ...
        $sql = "UPDATE emi_payments SET superapproval = '2', remarks = ? WHERE PaymentID = ?";
        $stmt = $conn->prepare($sql);
        if ($stmt === false) {
        ...
        if($row33['PaymentType']==='collect_payment'){

            $updateSql = "UPDATE emi_schedule SET PaidEMIs = PaidEMIs - 1,LastPaymentDate = oldlastpaymentdate, NextPaymentDate = oldnextpaymentdate WHERE LeadID = ? AND PaidEMIs > 0";
        ...
        elseif($row33['PaymentType']==='Partial Payment Adjusted in EMI'){
            $updateSql = "UPDATE emi_schedule SET PaidEMIs = PaidEMIs - 1,LastPaymentDate = oldlastpaymentdate, NextPaymentDate = oldnextpaymentdate WHERE LeadID = ? AND PaidEMIs > 0";
            $stmt2 = $conn->prepare($updateSql);
            if ($stmt2 === false) {
                throw new Exception($conn->error);
        ...
        elseif($row33['PaymentType']==='advance_emi'){
            $updateSql = "UPDATE emi_schedule SET PaidEMIs = PaidEMIs - 1,LastPaymentDate = oldlastpaymentdate, NextPaymentDate = oldnextpaymentdate WHERE LeadID = ? AND PaidEMIs > 0";
            $stmt2 = $conn->prepare($updateSql);
            if ($stmt2 === false) {
                throw new Exception($conn->error);
