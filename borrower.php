        ...
        <legend>Personal Information (Applicant Details) </legend>
         <div class="form-row">
          ...
          <div class="form-group col-md-4">
            ...
            <!--was readonly initially, project-->
    ...
    <fieldset>
        ...
        <label for="details2" class="-label">Work Details</label>
           ...
           <div class="form-group col-md-6 ">
                ...
                <label for="co_app" class="-label">CO-Applicant Score (CIBIL/EXP/EQI/CRIF)</label>
...
$('#loanForm').submit(function (e) {
    ...
    // Validate Aadhar, PAN, and phone number uniqueness
    $.ajax({
        url: 'validate.php',
        type: 'POST',
        data: { aadhar: aadharInput, pan: panInput, phone: phoneInput },
        dataType: 'json',
        success: function(response) {
            if (response.exists) {
                if (response.aadhar_exists) {
                    alert('Aadhar already exists in the database. Please enter a unique Aadhar number.');
                } else if (response.pan_exists) {
                    alert('PAN already exists in the database. Please enter a unique PAN number.');
                } else if (response.phone_exists) {
                    alert('Phone number already exists in the database. Please enter a unique phone number.');
                }
            } else {
        ...
        error: function() {
            alert('An error occurred while checking Aadhar, PAN, and phone number uniqueness. Please try again later.');
        }
    });
});
